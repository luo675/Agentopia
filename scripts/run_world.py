...
def main() -> None:
    ...
    import atexit
    from src.utils import flush_all_caches, project_root
    ...
    def _flush_and_exit(signum, frame):
        ...
        try:
            flush_all_caches()
            # Delete run cache instead of merging (same as normal exit path)
            run_cache_dir = project_root / "llm_cache" / data_dir
            if run_cache_dir.exists():
                import shutil as _shutil2
                _shutil2.rmtree(run_cache_dir)
    ...
    # 2. Delete run cache dir instead of merging
    # (fresh start each run — cross-run cache reuse is not useful here,
    #  and the merge loads all pickle files into memory, causing OOM on 16GB RAM)
    import shutil as _shutil
    run_cache_dir = project_root / "llm_cache" / data_dir
    if run_cache_dir.exists():
        _shutil.rmtree(run_cache_dir)
        print(f"  Deleted run cache: {run_cache_dir}")
