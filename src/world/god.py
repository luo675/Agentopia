...
def compress_events_to_l2(agent: "RoleAgent", year: int, week: int) -> str:
    ...
    Called from world.py in Settle stage when current_week > 5.
    Compresses W_{current_week-5} into a fading memory.

    Args:
        agent: The RoleAgent whose events to compress.
        year: The year of the week being compressed.
        week: The week being compressed.

    Returns:
        Compressed ~200-word summary string.
    """
    events = agent.dm.read_event_logs_for_week(year, week)
    if not events:
        return "No significant events that week."

    events_text = "\n".join(
        f"- [{e.get('type','')}] {e.get('activity_name','')}: {e.get('outcome_summary','')}"
        for e in events[:20]  # cap at 20 events
    )

    personality = agent.dm.read_profile().get("personality_traits", {}).get("qualitative", "")

    prompt = f"""## Task: Compress One Week of Life Events into a Fading Memory

{agent.name} is a person with the following personality: {personality}

Below are {agent.name}'s activities for Y{year}-W{week}. Write ~200 words summarizing the key moments. Focus on what would actually fade from a person's memory:
- What happened that week?
- Who did they spend time with?
- Any changes in relationships, feelings, or direction?

Do NOT include everything. Pick the most meaningful moments. This is what remains after a few weeks — the rest is forgotten.
...
Write the compressed memory only, no JSON, no tags, no meta-commentary. Use a natural narrative style."""
    ...
    if isinstance(response, str) and response and response != "NO_RESPONSE":
        return response[:800]  # cap at ~800 chars ≈ 200 words
    return "A quiet week."


def compress_l2_to_l3(agent: "RoleAgent", year: int, from_week: int, to_week: int) -> str:
    """Use God-model to compress a block of L2 memories into a ~50-word L3 milestone.

    Called from world.py in Settle stage when current_week > 20 and week % 5 == 0.
    Compresses L2 entries for weeks [from_week, to_week] into a distant memory.

    Args:
        agent: The RoleAgent whose L2 memories to compress.
        year: The year of the block.
        from_week: Start week of the block (inclusive).
        to_week: End week of the block (inclusive).

    Returns:
        ~50-word milestone string.
    """
    l2_entries = agent.dm.read_l2_memories(from_week=from_week, to_week=to_week)
    if not l2_entries:
        return "An unremarkable period."
    ...
    prompt = f"""## Task: Compress Five Weeks into a Distant Memory

{agent.name} is a person with personality: {personality}

Below are {agent.name}'s fading memories from Y{year} W{from_week}-W{to_week}. Compress this into ~50 words — a DISTANT memory. Like something a person would barely recall years later: just the ONE thing that stands out.
...
Write the milestone only, no JSON, no tags, no meta-commentary. One or two sentences — what would this person remember from this period years later?"""
    messages = [{"role": "user", "content": prompt}]
    from src.config import get_config as _get_config
    response = get_response_with_retry(
        post_processing_funcs=[],
    ...
    if isinstance(response, str) and response and response != "NO_RESPONSE":
        return response[:200]  # cap at ~200 chars ≈ 50 words
    return "An unremarkable period."


# ---- Emotional Seeds for Health Evaluation ----
# 50 spontaneous micro-emotions sampled from 14 psychological dimensions.
# Each week, one seed is randomly injected into the health evaluation prompt
# to break template behavior — same mechanism as T3A randomized Review prompt.
EMOTIONAL_SEEDS = [
    # -- Social Micro-Moments (most frequent in daily life) --
    "The barista remembered your order and it made you feel, for no good reason, like you existed somewhere.",
    "A stranger held the door and said 'take your time' in the gentlest voice. You thought about it all morning.",
    "Someone laughed at a joke you made and for three seconds, you were the undisputed center of the room.",
    "You waved at someone you thought you knew and they didn't wave back. Four hours later, you're still replaying it.",
    "A friend replied with just 'k.' and you spent twenty minutes decoding whether they were angry or just busy.",
    "Someone said 'see you later' in a tone that reminded you of someone you miss, and you nearly responded in their language.",

    # -- Somatic Moods (body-driven) --
    "You haven't eaten since breakfast and the world has started to feel slightly hostile — everything is too loud.",
    "Third cup of coffee and you can feel your heartbeat in your fingertips. Not anxious, just... running too fast.",
    "Stretching on the sofa, your spine cracked and a wave of pure animal satisfaction washed through you.",
    "The temperature dropped suddenly and you felt a protective urge to curl into yourself like a pill bug.",
    "A deep breath filled your lungs completely and you felt spacious inside for the first time all day.",

    # -- Existential Glimpses --
    "You were washing dishes and thought: is this the whole shape of it? Then the soap bubble popped.",
    "The moon looked close tonight. You felt very small, but not in a bad way.",
    "You looked at your hands under the kitchen light and realized they look like your father's hands, and he's gone.",
    "For about ten seconds while brushing your teeth, you were completely certain that nothing you do will be remembered.",

    # -- Memory Intrusion --
    "A song you haven't heard since high school came on and suddenly you were twenty again for three seconds.",
    "You woke up with a feeling from a dream you couldn't remember — just this heavy warmth in your chest.",
    "You were folding laundry and suddenly felt guilty about something you said to a friend three years ago.",
    "The smell of rain on pavement brought back a memory you can't quite place.",

    # -- Ambient Loneliness --
    "You ate dinner by yourself and felt, between bites, how quiet your life is. Not sad quiet. Just quiet.",
    "You laughed at something on your phone and the laugh echoed in the empty room. That echo was louder.",
    "Walking home alone, the city was beautiful and you wanted to turn to someone and say 'look' — but there was no one.",

    # -- Mundane Sublime --
    "The late-afternoon light came through the blinds and made stripes across your desk. You felt a peace you didn't earn.",
    "A pigeon landed on the windowsill and tilted its head and you felt seen. By a pigeon. And it counted.",
    "The way the steam rose from your tea in the morning light — it looked like calligraphy for about three seconds.",
    "Green lights all the way from 4th to 9th. The city was conspiring in your favor.",

    # -- Self-Image Flickers --
    "You caught your reflection in a dark window and for a second looked like someone who had their life together.",
    "You said something articulate and could feel yourself becoming the kind of person who says articulate things.",
    "You re-read an email you'd sent and cringed. Who writes like that? You sounded desperate and earnest.",
    "Someone called you 'kind' yesterday and you've been quietly reheating that word ever since.",

    # -- Phantom Obligation --
    "It's evening and you have the feeling you were supposed to do something today. You can't figure out what.",
    "You checked your email, calendar, messages — nothing pending. Still feel like you're forgetting something.",
    "Every time you sit down to relax, a voice says you should be doing something. It never says what.",

    # -- Counterfactual Drift --
    "You drove past a house in a town you almost moved to and spent the next three blocks living a life you never had.",
    "You imagined what your life would be like if you'd never stopped doing that thing you used to love.",
    "For a few seconds you were certain that somewhere, in some branching of reality, you're a completely different person. You could feel it.",

    # -- Impulse Friction --
    "You wanted to tell the person next to you that you liked their scarf but the moment passed. You carried the unsaid words.",
    "You almost texted someone and then didn't. The not-texting felt heavier than the texting would have.",
    "You had the perfect comeback three minutes after the conversation ended. The frustration lingered like a phantom limb.",

    # -- Relational Undertow --
    "You thought about a friend you haven't spoken to in two years while making toast. You wonder if they make toast the same way.",
    "Out of nowhere, you felt grateful for a teacher you had when you were twelve. You don't remember her name, but she was kind.",
    "Someone you used to be close with posted a photo looking happy and you felt something you can't name — not jealousy, not sadness.",

    # -- Digital Residue --
    "You spent an hour scrolling and now feel full and empty at the same time, like you ate a meal made entirely of garnishes.",
    "You closed your laptop and the silence in the room felt louder than usual, like your brain was still processing text-conversations.",
    "You opened your phone to check the time and closed it forty minutes later with no memory of what you looked at.",

    # -- Temporal Disorientation --
    "Someone said what month it is and you felt the year rearrange itself inside you like furniture being moved.",
    "You looked at the clock expecting it to be late and it wasn't. Those extra hours felt like a gift you didn't know how to use.",
    "Sunday afternoon and the weekend is dissolving. You can feel it going, like sand through a fist.",
]

def evaluate_agent_health(agent: "RoleAgent", year: int, week: int) -> dict:
    """Use God-model to evaluate how this week's events affected the agent's health.

    Reads the agent's event_log for the given week, applies HEALTH_RULES,
    and returns physical_delta, mental_delta, and a narrative summary.

    Called from world.py in Settle stage, after memory compression.

    Args:
        agent: The RoleAgent to evaluate.
        year: Current year.
        week: Current week.

    Returns:
        {"physical_delta": int, "mental_delta": int, "narrative": str}
    """
    from src.config import get_config as _get_config
    from src.utils import get_response_with_retry
    from src.agents.prompts import HEALTH_RULES, HEALTH_EVAL_PROMPT

    # Read this week's event log
    events = agent.dm.read_event_logs_for_week(year, week)
    if not events:
        return {"physical_delta": 0, "mental_delta": 0, "narrative": "No events this week."}

    event_lines = []
    for e in events:
        evt_type = e.get('type', 'activity')
        evt_name = e.get('activity_name', '')
        evt_summary = e.get('outcome_summary', e.get('content', ''))
        label = f"[{evt_type}]" if not evt_name else f"[{evt_type}] {evt_name}"
        event_lines.append(f"- {label}: {evt_summary}")
    # If only a trivial weekly_summary, note that the week had routine activity
    if len(event_lines) <= 1:
        event_lines.append("- [info] This was a routine week with daily activities across 4 time blocks (NIGHT/MORNING/AFTERNOON/EVENING). Solo activities, encounters, and social interactions occurred as normal.")
    event_log_text = "\n".join(event_lines) if event_lines else "No events recorded."

    # Current health
    physical, mental = agent.dm.get_current_health()

    # Personality for context
    profile = agent.dm.read_profile()
    qualitative = profile.get("personality_traits", {}).get("qualitative", "")
    personality_summary = f"{agent.name}: {qualitative}" if qualitative else agent.name

    # Build prompt
    prompt = HEALTH_RULES + "\n\n" + HEALTH_EVAL_PROMPT.format(
        personality_summary=personality_summary,
        event_log_text=event_log_text,
        physical_health=physical,
        mental_health=mental,
    )
    # Inject a random emotional seed to break template behavior.
    # Same mechanism as T3A randomized Review prompt: a different
    # subjective anchor each week prevents the model from finding
    # a single high-probability template and locking onto it.
    import random
    seed = random.choice(EMOTIONAL_SEEDS)
    prompt += (
        f"\n\n### This Week's Internal Context\n\n"
        f"{seed}\n\n"
        f"Incorporate this subtle emotional undertone into your evaluation. "
        f"Do NOT mention the seed explicitly in the narrative. Let it color "
        f"your judgment naturally — like a background mood that slightly "
        f"shifts how this week's events are perceived."
    )
    messages = [{"role": "user", "content": prompt}]
    ...
    if isinstance(response, str) and response and response != "NO_RESPONSE":
        match = re.search(
            r'\{[^{}]*"physical_delta"[^{}]*"mental_delta"[^{}]*"narrative"[^{}]*\}',
            response, re.DOTALL
        )
        if match:
            try:
                ...
                return {
                    "physical_delta": int(data.get("physical_delta", 0)),
                    "mental_delta": int(data.get("mental_delta", 0)),
                    "narrative": str(data.get("narrative", ""))[:200],
                }
            except (json.JSONDecodeError, ValueError, KeyError):
                pass

    # Fallback: no health change
    return {"physical_delta": 0, "mental_delta": 0, "narrative": "Health evaluation unavailable."}
