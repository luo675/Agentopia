...
class World:
    ...
    def _load_or_assign_models(self, names: List[str]) -> Dict[str, str]:
        ...
        if path.exists():
            ...
            # Auto-assign missing agents on resume (e.g. NPCs added mid-sim)
            missing = [n for n in names if n not in assignment]
            if missing:
                rng = random.Random(self.data_dir)
                models = self._role_models
                for name in missing:
                    assignment[name] = models[len(assignment) % len(models)]
                self.logger.info(f"Auto-assigned {len(missing)} missing agents on resume")
    ...
    def step(self) -> None:
        ...
        for day in range(1, self.config["time"]["n_day"] + 1):
            ...
            # Trim daily context to prevent unbounded prompt growth from 4 time blocks
            for agent in self.agents:
                if agent.deceased:
                    continue
                agent.trim_daily_context()
            ...
            if self.parallel:
                ...
                with ThreadPoolExecutor(max_workers=total_tasks) as ex:
                    ...
                    # Note: same-day memory not tracked in parallel mode —
                    # activities may run concurrently and out of order.
                ...
                # Solo activities — pass same-day earlier-block memory
                daily_context: dict[str, str] = {}
                for act in solo_acts:
                    agent_name = act.agents[0].name
                    act.earlier_today = daily_context.get(agent_name, "")
                    # First solo block of the day → full roleplay context.
                    # Later blocks → lightweight (persona only) to save ~4000 tokens per block.
                    act.is_first_block = (daily_context.get(agent_name, "") == "")
                    act.run()
                    if act.last_summary:
                        prev = daily_context.get(agent_name, "")
                        daily_context[agent_name] = prev + "\n" + act.last_summary if prev else act.last_summary
        ...
        if self.parallel:
            with ThreadPoolExecutor(max_workers=pool_size(len(self.agents))) as ex:
                list(ex.map(lambda a: a.review(), [a for a in self.agents if not a.deceased]))
        else:
            for agent in self.agents:
                if agent.deceased:
                    continue
        ...
        # ---- Memory Compression (L2/L3) ----
        # Controlled by config.world.memory.enable_layered (ablation flag).
        # When false, skip all compression — agent only has 5-week sliding window.
        from src.config import get_config
        cfg = get_config()
        if cfg.get("world", {}).get("memory", {}).get("enable_layered", True):
            # L2: compress W_{week-5} to ~200-word summary
            if t.week > 5:
                from src.world.god import compress_events_to_l2
                for agent in self.agents:
                    try:
                        l2 = compress_events_to_l2(agent, t.year, t.week - 5)
                        agent.dm.save_l2_memory(t.year, t.week - 5, l2)
                    except Exception as e:
                        self.logger.warning(f"L2 compress failed for {agent.name}: {e}")
            # L3: compress 5-week L2 block to ~50-word milestone
            if t.week > 20 and t.week % 5 == 0:
                from src.world.god import compress_l2_to_l3
                for agent in self.agents:
                    try:
                        l3 = compress_l2_to_l3(agent, t.year, t.week - 20, t.week - 15)
                        agent.dm.save_l3_memory(t.year, t.week - 20, t.week - 15, l3)
                    except Exception as e:
                        self.logger.warning(f"L3 compress failed for {agent.name}: {e}")

        # ---- Health Evaluation (⑤) ----
        health_cfg = cfg.get("world", {}).get("health", {})
        if health_cfg.get("enable", False):
            from src.world.god import evaluate_agent_health
            for agent in self.agents:
                if agent.deceased:
                    continue
                try:
                    result = evaluate_agent_health(agent, t.year, t.week)
                    physical_new = max(0, min(100,
                        agent.dm.get_current_health()[0] + result["physical_delta"]))
                    mental_new = max(0, min(100,
                        agent.dm.get_current_health()[1] + result["mental_delta"]))
                    agent.dm.save_health_state(
                        t.year, t.week, physical_new, mental_new, result.get("narrative", ""))
                    agent._check_life_crisis()
                    if agent.deceased:
                        self.logger.warning(
                            f"💀 {agent.name} has died at Y{t.year} W{t.week}."
                        )
                    # World-level crisis visibility (agent logger writes to per-agent file)
                    if mental_new <= health_cfg.get("crisis_mental_threshold", 20):
                        self.logger.warning(
                            f"[HEALTH-CRISIS] {agent.name}: mental={mental_new} (critical)"
                        )
                    if physical_new <= health_cfg.get("crisis_physical_threshold", 10):
                        self.logger.warning(
                            f"[HEALTH-CRISIS] {agent.name}: physical={physical_new} (critical)"
                        )
                except Exception as e:
                    self.logger.warning(f"Health eval failed for {agent.name}: {e}")

        # ---- Write activity event logs ----
        self._write_activity_event_logs(t)

    # Memory Compress/Event Log Helpers ---------------------------------------

    def _write_activity_event_logs(self, t: TimeState) -> None:
        """After Settle, write event log entries for all agents' activities this week.

        For each agent, records a weekly event log entry summarizing their
        solo/joint/public/encounter activities for the week.
        This is the primary data source for L1 memory (not a diary written by agent).
        """
        for agent in self.agents:
            try:
                agent.dm.write_event_log({
                    ...
                    "week": t.week,
                    "type": "weekly_summary",
                    "activity_name": "Week Summary",
                    "outcome_summary": f"Completed week {t.week}.",
                })
            except Exception as e:
                self.logger.warning(
                    f"Event log write failed for {agent.name}: {e}"
                )
