...
class SoloActivity(Activity):
    ...
    Attributes:
        time_block: Which time block (MORNING/AFTERNOON/EVENING) this activity occurs in.
    """

    time_block: str = "AFTERNOON"
    earlier_today: str = ""  # summary of earlier time blocks today (injected into prompt)
    last_summary: str = ""   # captured after run(), for feeding into next block
    is_first_block: bool = True  # True = full roleplay context; False = lightweight (persona only)
    ...
    def __post_init__(self) -> None:
        ...
        if self.activity_id is None:
            ...
            identifier = f"{self.agents[0].name}-{self.time_block}"
            self.activity_id = make_activity_id("solo", self.time, identifier)
    ...
    def run(self) -> None:
        ...
        agent.enter_solo_activity(time_block=self.time_block, earlier_today=self.earlier_today, is_first_block=self.is_first_block)
        if logger:
            logger.info(
        ...
        # Capture brief summary for same-day memory passing to later time blocks
        self.last_summary = f"[{self.time_block}] {activity_content[:120]}"
        if logger:
            logger.info(
