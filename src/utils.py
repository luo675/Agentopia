...
def _get_response(model: str, messages, max_tokens=None, **kwargs):
    ...
    elif not isinstance(out, list) or not out:
        out = [{"role": "assistant", "content": str(out)}]
...
def flush_all_caches() -> None:
    ...
    # Clear delta registries to free memory
    import gc as _gc
    _main_thread_delta.clear()
    with _delta_registry_lock:
        _delta_registry.clear()
...
def _merge_worker_caches(base_cache_file: str) -> bool:
    ...
    for shard_file in shard_files:
        try:
            ...
            merged_count += 1
            del shard_cache  # Free memory immediately after merge
        except Exception as e:
...
def _merge_cache_file(src: Path, dst: Path) -> tuple[int, int]:
    ...
    del src_cache  # Free before writing — avoid holding two dicts in memory
...
def merge_run_cache(run_data_dir: str, delete_after: bool = True) -> bool:
    ...
    for pkl_file in run_cache_dir.glob(".cache*.pkl"):
        ...
        import gc as _gc
        _gc.collect()  # Free memory between cache file merges
