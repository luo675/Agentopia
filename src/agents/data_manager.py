...
class DataManager:
    ...
    def __post_init__(self) -> None:
        ...
        self.event_log = self.memory / "event_log.jsonl"
        self.l2_memory_dir = self.memory / "l2"
        self.l3_memory_dir = self.memory / "l3"
    ...
    def activity_prompt(
        ...
        if activity_type == "joint":
            ...
            ACTIVITY_PROMPT = SOLO_ACTIVITY_PROMPT.format(time_block=time_block)
    ...
    def recent_history_prompt(self) -> str:
        ...
        layered = self.read_layered_memory()
        if layered:
    ...
    def list_scratchpads(
        self,
        character_limit: Optional[int] = 20,  # reduced from 50 — social knowledge decays like memory (③)
        ...
        - character_limit: max number of character scratchpads to show (default 20).
    ...
    def read_message(self) -> str:
        ...
        n_weeks = min(2, int(config["world"]["contact"]["n_prev_week_contact_history"]))  # capped at 2 — social decay (③)
        lines: List[str] = [
        ...
        for who in peers:
            ...
            # Scratchpad summary already loaded by roleplay_prompt → list_scratchpads.
            # Only load public info here to avoid duplication and prompt bloat.
            pub_info = self.read_others_pub_info(who)
    ...
    def read_weekly_summaries(self, n_weeks: int = 4) -> List[Dict]:
        """DEPRECATED: Use read_layered_memory() instead. Returns entries with time + content for backward compat."""
        events = self.read_event_logs(n_weeks=n_weeks)
        return [{"time": e["time"], "content": e.get("outcome_summary", "")} for e in events]
...
# ============ Memory System (Event Log + L2 + L3) ============
    ...
    def read_event_logs_for_week(self, year: int, week: int) -> List[Dict]:
        """Read ALL event log entries for a specific week (for God compression).

        Uses a generous window (52 weeks = 1 full year) to ensure events from very
        early weeks are still reachable during long simulations (e.g. 52-week runs).
        """
        if not self.event_log.exists():
            return []
        entries = self._read_jsonl(self.event_log, max_weeks=52, exclude_cur_t=False)
        return [e for e in entries if e.get('year') == year and e.get('week') == week]

    def save_l2_memory(self, year: int, week: int, compressed: str) -> None:
        """Save a God-compressed L2 summary of one week's events."""
        import json

        dir_path = self.l2_memory_dir / f"year={year}"
        dir_path.mkdir(parents=True, exist_ok=True)
        path = dir_path / f"week={week:02d}.json"
        path.write_text(
            json.dumps({"year": year, "week": week, "compressed": compressed},
                       ensure_ascii=False),
            encoding="utf-8"
        )
    ...
    def read_l2_memories(self, from_week: int, to_week: int) -> List[Dict]:
        ...
        Searches backwards through years (current → year-1 → year-2) to find
        L2 files. The 3-year window is sufficient because the L2 range is at
        most ~2 years wide (20 weeks ÷ n_weeks_per_year).
        """
        import json
        ...
        for w in range(from_week, to_week + 1):
            for year_offset in range(3):  # current year → 2 years back
                path = self.l2_memory_dir / f"year={t.year - year_offset}" / f"week={w:02d}.json"
                if path.exists():
                    results.append(json.loads(path.read_text(encoding="utf-8")))
                    break
        return sorted(results, key=lambda x: (x.get("year", 0), x.get("week", 0)))
    ...
    def save_l3_memory(self, year: int, from_week: int, to_week: int, milestone: str) -> None:
        ...
        dir_path = self.l3_memory_dir / f"year={year}"
        dir_path.mkdir(parents=True, exist_ok=True)
        path = dir_path / f"w{from_week:02d}-{to_week:02d}.json"
        path.write_text(
            json.dumps({"year": year, "from_week": from_week, "to_week": to_week,
                       "milestone": milestone}, ensure_ascii=False),
            encoding="utf-8"
        )
    ...
    def health_dir(self) -> Path:
        p = self.memory / "health"
        p.mkdir(parents=True, exist_ok=True)
        return p

    def save_health_state(self, year: int, week: int, physical: int, mental: int,
                          narrative: str = "") -> None:
        """Save health state after God evaluation."""
        import json
        path = self.health_dir / f"year={year}" / f"week={week}.json"
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps({
            "year": year, "week": week,
            "physical_health": max(0, min(100, physical)),
            "mental_health": max(0, min(100, mental)),
            "narrative": narrative,
        }, ensure_ascii=False), encoding="utf-8")

    def read_health_history(self, n_weeks: int = 10) -> List[Dict]:
        """Read recent health states, newest first (sorted by year, week numerically)."""
        import json
        if not self.health_dir.exists():
            return []

        def _parse_health_path(f: Path) -> tuple:
            """Extract (year, week) from path like .../year=2020/week=5.json"""
            week = int(f.stem.split("=")[-1])
            year = int(f.parent.name.split("=")[-1])
            return (year, week)

        files = sorted(self.health_dir.rglob("*.json"),
                       key=_parse_health_path, reverse=True)
        results = []
        for f in files[:n_weeks]:
            results.append(json.loads(f.read_text(encoding="utf-8")))
        return results

    def get_current_health(self) -> tuple:
        """Return (physical_health, mental_health) from most recent record.
        If no record exists, return initial values from config."""
        history = self.read_health_history(n_weeks=1)
        if history:
            h = history[0]
            return h["physical_health"], h["mental_health"]
        from src.config import get_config
        cfg = get_config()
        hc = cfg.get("world", {}).get("health", {})
        return hc.get("initial_physical", 80), hc.get("initial_mental", 70)

    def get_health_consciousness(self) -> int:
        """Derived from personality: (responsibility + control + judging) / 3.
        High → proactive about health. Low → ignores health signals."""
        profile = self.read_profile()
        pt = profile.get("personality_traits", {}).get("quantitative", {})
        return (pt.get("responsibility", 50) + pt.get("control", 50) + pt.get("judging", 50)) // 3

    def get_emotional_stability(self) -> int:
        """Derived from personality: (confidence + patience) / 2.
        High → resilient, recovers quickly. Low → neurotic, fragile."""
        profile = self.read_profile()
        pt = profile.get("personality_traits", {}).get("quantitative", {})
        return (pt.get("confidence", 50) + pt.get("patience", 50)) // 2

    def get_social_seeking(self) -> int:
        """Derived from personality: extraversion or (100 - introversion).
        High → seeks social support when struggling. Low → isolates."""
        profile = self.read_profile()
        pt = profile.get("personality_traits", {}).get("quantitative", {})
        if "extraversion" in pt:
            return pt["extraversion"]
        return 100 - pt.get("introversion", 50)

    def health_awareness_text(self) -> str:
        """Generate natural-language health perception for Plan/Review prompts.
        Returns empty string if agent feels fine (both > 70).
        Modulated by health_consciousness: high → more urgent wording."""
        physical, mental = self.get_current_health()
        consciousness = self.get_health_consciousness()

        lines = []
        for value, dimension in [(physical, "physical"), (mental, "mental")]:
            if value >= 70:
                continue  # feels normal, no mention
            elif value >= 40:
                if dimension == "physical":
                    if consciousness >= 75:
                        lines.append("You've been feeling somewhat tired lately and know you should rest more.")
                    elif consciousness < 30:
                        lines.append("You've been feeling somewhat tired lately.")
                    else:
                        lines.append("You've been feeling somewhat tired lately.")
                else:
                    if consciousness >= 75:
                        lines.append("There's a quiet heaviness you've been carrying — you should talk to someone.")
                    elif consciousness < 30:
                        lines.append("There's a quiet heaviness you've been carrying.")
                    else:
                        lines.append("There's a quiet heaviness you've been carrying.")
            elif value >= 20:
                if dimension == "physical":
                    if consciousness >= 75:
                        lines.append("Your body feels consistently drained. You urgently need to rest and take care of yourself.")
                    else:
                        lines.append("Your body feels consistently drained. Physical tasks take real effort now.")
                else:
                    if consciousness >= 75:
                        lines.append("A persistent sense of emptiness weighs on you. You know you need to reach out to someone.")
                    else:
                        lines.append("A persistent sense of emptiness weighs on you. Motivation feels distant.")
            else:  # < 20
                if dimension == "physical":
                    lines.append("You feel physically broken. Even basic daily tasks require enormous effort.")
                else:
                    if self.get_social_seeking() >= 60:
                        lines.append("Everything feels gray and meaningless. Perhaps someone could help you carry this.")
                    else:
                        lines.append("Everything feels gray and meaningless. You struggle to find reasons to continue.")

        return " ".join(lines) if lines else ""

    def _build_sliding_window_memory(self) -> str:
        """Fallback: 5-week sliding window memory (pre-③ behavior).

        Used for ablation experiments when config.world.memory.enable_layered=false.
        Returns only recent 5-week event logs without the L2/L3 compression layers,
        matching the baseline memory architecture.
        """
        summaries = self.read_weekly_summaries(n_weeks=5)
        if summaries:
            lines = []
            for s in summaries:
                lines.append(
                    f"- {s.get('time','')}: {s.get('content','')}"
                )
            return "## What You Remember\n\nHere is what you recall about your recent life:\n\n" + "\n".join(lines)
        return "## What You Remember\n\n(You just started your life here. No memories yet.)"
    ...
    def read_layered_memory(self) -> str:
        ...
        and formats them into the MEMORY_CONTEXT template.

        When config.world.memory.enable_layered is false (ablation mode), falls back
        to a 5-week sliding window only — the pre-③ behavior.
        """
        from src.config import get_config
        cfg = get_config()

        # ---- Ablation: 5-week sliding window when layered memory is off ----
        if not cfg.get("world", {}).get("memory", {}).get("enable_layered", True):
            return self._build_sliding_window_memory()

        t = self.clock.get_time()
        from src.agents.prompts import MEMORY_CONTEXT
        n_weeks_per_year = cfg["world"]["time"]["n_week"]
        ...
        if events:
            ...
            for e in events[:30]:
                l1_lines.append(
                    f"- W{e.get('week', '?')}: {e.get('type','')} "
                    f"'{e.get('activity_name','')}' — {e.get('outcome_summary','')}"
                )
            l1_text = "\n".join(l1_lines)
        else:
            l1_text = "(No recent events recorded yet. You just started your life here.)"

        # L2: Fading memories — compressed summaries (6-20 weeks ago)
        # Use absolute-week arithmetic relative to simulation start to handle
        # year boundaries correctly (e.g. year-1 weeks near year-2 start).
        start_year = cfg["world"]["time"]["start_year"]
        cur_abs = (t.year - start_year) * n_weeks_per_year + t.week
        l2_end_abs = cur_abs - 6
        l2_start_abs = max(1, cur_abs - 20)

        if l2_end_abs >= 1:
            fw = ((l2_start_abs - 1) % n_weeks_per_year) + 1
            tw = ((l2_end_abs - 1) % n_weeks_per_year) + 1
            l2_entries = self.read_l2_memories(from_week=fw, to_week=tw)
        else:
            l2_entries = []
        if l2_entries:
        ...
        if l3_entries:
            l3_lines = [
                f"Y{e['year']} W{e['from_week']}-{e['to_week']}: {e['milestone']}"
                for e in l3_entries
            ]
            l3_text = "\n".join(l3_lines)
        else:
            l3_text = "(No distant memories yet — your life here is just beginning.)"
