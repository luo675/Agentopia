...
- Each day has four time blocks: NIGHT (00:00-06:00), MORNING (06:00-12:00), AFTERNOON (12:00-18:00), and EVENING (18:00-24:00). Every person can spend each block alone (solo activity) or with others (joint activity). NIGHT is typically for sleep, but you may stay up late working, go out, or do anything else — there are no forced sleep rules.
- Each week is a cycle, divided into the following phases in order:
    ...
    5. **Activity**: The week runs from Day 1 to Day {config["world"]["time"]["n_day"]}. Each day, you have four activity blocks: NIGHT, MORNING, AFTERNOON, EVENING. Each block can be a different activity.
...
You should check your existing schedule, and outline your general weekly plan, and then plan activities for each day's four time blocks: NIGHT, MORNING, AFTERNOON, and EVENING. For each day, describe what you plan to do in each block. NIGHT can be for sleep, but you may also stay up working, go out, or do whatever fits your character. Your plan should cover all 5 days × 4 blocks.
...
REVIEW_PROMPT = """## Weekly Review: Personal Reflection

This is your weekly review — just your personal thoughts. The system tracks what you did, so there is no need to write a factual summary. Instead, reflect on these questions:
...
This is your **{time_block}** time block (about 2-3 hours). You may choose one activity based on your own will and your weekly plan. This includes (but not limited to) learning and self-improvement, extra work, shopping, leisure and entertainment.

**Time block context:**
- NIGHT: Late hours — most people sleep, but you might stay up working, go out, have deep conversations, or wrestle with insomnia. Your energy may be lower.
- MORNING: Start of day — you might exercise, study, run errands, or sleep in if you were up late.
- AFTERNOON: Main productive hours — work, deep focus, important tasks.
- EVENING: Wind-down time — social activities, relaxation, hobbies, reflection.

Important notes:
- You can only choose ONE activity for this time block - do not split this time across multiple activities.
- You should specify the expected activity content. For example, what subject you will study or what work you will do.
- If you stayed up late or woke up early, your energy level should reflect that.
...
SCORE_ONE_PERSON_PROMPT = """## Private Evaluation of One Person

This is a COMPLETELY PRIVATE evaluation. Be completely honest and do not overthink — just report how you genuinely feel.

## Person You Are Evaluating: {person_name}

## Your Interactions With {person_name}

{interactions}

## Your Task

Score {person_name} on TWO dimensions, each from 0 to 100:

- **Affection**: How much you **personally like** them. (0 = hatred/contempt, 50 = neutral, 100 = deep affection/love)
- **Respect**: How much you **admire** their abilities and character. (0 = contempt, 50 = neutral, 100 = great admiration)

Evaluate against the 50 baseline. You are scoring ONE person — do NOT compare against others.

## Output Format

Return ONLY a JSON object (no explanation, no commentary):

{{"affection": <0-100>, "respect": <0-100>}}
"""

HEALTH_RULES = """## Health Impact Rules

Evaluate this week's events against these rules. Stay within the given ranges.

### Physical Health (0-100 scale)
- No rest/sleep across all 4 blocks in a day: -2 to -3 per sleepless day
- Slept during MORNING or AFTERNOON after a sleepless NIGHT: halve that day's penalty
- Physical exercise or outdoor activity mentioned: +1 to +2 per occurrence
- Injury, illness, or accident mentioned in events: -5 to -15
- Consistent healthy routine (sleep + exercise across blocks): +1 to +2 bonus for the week

### Mental Health — Social (0-100 scale)

#### Daily Social Baseline (MOST COMMON case)
- Routine greetings, casual chats, brief encounters, small talk: **0 (neutral)**
- This is normal human life. These maintain equilibrium but don't meaningfully boost mood.
- Even a single brief interaction counts as routine contact — NOT "no social contact."

#### True Isolation (RARE — only when event_log shows ZERO interactions)
- No interaction with ANY person in ANY block this entire week: **-3 to -5**
- Check carefully: if the agent spoke to ANYONE (encounter, joint activity, contact, even a barista), this does NOT apply.

#### Deep Connection
- Meaningful conversation, emotional vulnerability, receiving help: **+3 to +8**
- Successful collaboration toward a shared goal: **+5 to +10**
- Being thanked, appreciated, or recognized by someone: **+3 to +6**

#### Social Conflict
- Minor disagreement, misunderstanding, tense exchange: **-2 to -5** (BOTH parties affected)
- Serious argument, betrayal, broken trust: **-8 to -15** (BOTH parties)
- Public humiliation, reputation damage: **-10 to -20** (primarily the victim)
- IMPORTANT: When two people conflict, BOTH take a mental hit. The aggressor's penalty may be smaller than the victim's, but is never zero.
- If reconciliation occurs later between the same people: **+3 to +6** (partial restoration for both)

### Mental Health — Non-Social (0-100 scale)
- Major failure (job loss, rejection, public embarrassment): -10 to -25
- Achievement (goal completed, recognition, praise): +5 to +12
- Multiple negative events compound up to -35 total for mental in one week

### Natural Recovery (apply AFTER event-based scoring)
- If mental_health is currently 30-60: **+1 to +3** (the mind naturally seeks equilibrium)
- If mental_health is currently 10-30: **+2 to +5** (stronger recovery drive, but don't overdo it — depression limits recovery)
- If mental_health is currently 0-10: **+1 to +2** (severe depression limits natural recovery, but some resilience remains)
- If mental_health is currently 0: do NOT give zero delta. Give at least **+1** (you cannot go below 0 — the floor has been reached, and tiny upward movement is the only direction possible)

### Baseline Fluctuation
- Even in uneventful weeks:
  - Physical: -1 to +1 (natural fatigue/recovery cycle)
  - Mental: -2 to +2 (normal mood variation)
- Add baseline fluctuation ON TOP of event-based scores and natural recovery.

### Narrative Guidelines
- Do NOT repeat the same description every week. Vary your wording.
- If the same pattern has continued for 3+ weeks, write: "Continued [pattern]. [brief new observation]." Do not re-describe the cause.
- If health was already at 0 and no major new negative event occurred: "Mental health remains critically low. [one specific detail from this week, if any]."
- If this was a routine week: "A routine week — health held steady with minor fluctuations."

### Rules
- Each event counted once. Do not double-count.
- The agent's current health limits apply: physical/mental cannot go below 0 or above 100.
- A week with routine social contact and no major events should typically produce a mental_delta near 0 (range: -2 to +2 after baseline fluctuation)."""

HEALTH_EVAL_PROMPT = """## Weekly Health Assessment

Review the agent's event log for this week. Evaluate how events affected their physical and mental health.
...
### Current Health
Physical: {physical_health}/100 | Mental: {mental_health}/100

### Task
Apply the Health Impact Rules above. Score each relevant event, then sum to a total physical_delta and mental_delta for the week. Write a one-sentence narrative describing the key health factor this week. Follow the Narrative Guidelines strictly.

### Output Format
Return ONLY a JSON object (no explanation):
{{"physical_delta": <int>, "mental_delta": <int>, "narrative": "<one sentence>"}}"""

HEALTH_AWARENESS = """## Health Awareness Levels

Generate a single brief sentence describing how the agent FEELS physically and mentally. Use these levels:

### Physical
- 70+: "" (no mention — feels normal, omit entirely)
- 40-69: "You've been feeling somewhat tired lately."
- 20-39: "Your body feels consistently drained. Physical tasks take real effort now."
- <20: "You feel physically broken. Even basic daily tasks require enormous effort."

### Mental
- 70+: "" (no mention — feels normal, omit entirely)
- 40-69: "There's a quiet heaviness you've been carrying."
- 20-39: "A persistent sense of emptiness weighs on you. Motivation feels distant."
- <20: "Everything feels gray and meaningless. You struggle to find reasons to continue."

### Intensity Modulation by Personality
- High conscientiousness (75+): use the MORE urgent sentence for this level
- Low conscientiousness (<30): use the LESS urgent sentence for this level
- Otherwise: use the standard sentence

### Output Rules
- Return ONLY the two sentences (or just one if only one dimension is below 70). No labels, no "Physical:" prefix.
- If both dimensions are 70+: return "" (empty string — the agent feels fine).
- The sentences should read as natural internal monologue, not clinical diagnosis."""

MEMORY_CONTEXT = """## What You Remember

Here is what you recall about your recent life. This is based on events that happened, not on any diary you wrote — because you do not keep a diary. Like a real person, recent events are vivid, older events are fuzzy, and the distant past is only landmarks.

Recent Weeks (vivid, 1-5 weeks ago):
{l1_memory}

Earlier Weeks (fading, 6-20 weeks ago):
{l2_memory}

Distant Past (only major moments, 21+ weeks ago):
{l3_memory}
"""
