...
class RoleAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self.config = config
        self.deceased = False  # health system: marked True if agent dies
    ...
    def review(self) -> None:
        """Weekly review: generate personal reflection and save to event log."""
        t = self.clock.get_time()
        ...
        # Parse and filter: keep only Reflection, remove Thinking (no Summary)
        content = self._parse_review_response(final_text)

        # Save reflection as event log entry (not as diary written by agent)
        if content and not self.no_context_engineering:
            self.dm.write_event_log({
                ...
                "week": t.week,
                "type": "weekly_review",
                "activity_name": "Weekly Reflection",
                "outcome_summary": content[:500],
            })

        self.logger.info(f"[REVIEW][year={t.year} week={t.week}] completed")

    def _parse_review_response(self, response: str) -> str:
        """Parse review response: keep from 'Reflection:' onwards, filter out Thinking."""
        import re
    ...
    def trim_daily_context(self) -> None:
        """Clear accumulated daily chatter to prevent prompt explosion from 4 time blocks.

        Rebuilds activity_context from stored state (scratchpad), which keeps the Plan
        and persona but drops raw Contact and Activity conversation history from prior days.
        """
        self.activity_context = self.dm.roleplay_prompt()

    def enter_solo_activity(self, time_block: str = "AFTERNOON", earlier_today: str = "", is_first_block: bool = True) -> None:
        ...
        Args:
            time_block: Which time block this activity is in (NIGHT/MORNING/AFTERNOON/EVENING)
            earlier_today: Summary of what the agent did in earlier time blocks today
            is_first_block: If True, load full roleplay context (persona + scratchpad + memory + state).
                           If False, load lightweight context (persona + worldview + commonsense only)
                           to save token budget for later time blocks in the same day.
        """
        t = self.clock.get_time()
        self.logger.info(
            f"[SOLO_ACTIVITY][year={t.year} week={t.week} day={t.day} block={time_block} person={self.name}] enter solo activity"
        )

        # Build initial context for this activity
        if is_first_block:
            # Full context: persona + scratchpad + layered memory + state + schedule + worldview
            roleplay_ctx = self.dm.roleplay_prompt()
        else:
            # Lightweight context: persona + worldview + commonsense only.
            # Scratchpad, layered memory, schedule, and state haven't changed since
            # the first block this morning — reloading them wastes ~4000 tokens per block.
            from src.agents.prompts import WORLDVIEW, COMMONSENSE

            persona = self.dm.character_prompt()
            light_parts = [persona, WORLDVIEW, COMMONSENSE]
            light_prompt = "\n\n".join([p.strip() for p in light_parts if p.strip() != ""])
            roleplay_ctx = [{"role": "system", "content": light_prompt}]

        base_prompt = roleplay_ctx + self.dm.activity_prompt(
            activity_type="solo", time_block=time_block
        )

        if earlier_today:
            earlier_blk = [{
                "role": "user",
                "content": f"## Earlier Today\n\nHere is what you have done so far today:\n\n{earlier_today}\n\nNow continue with your {time_block} block."
            }]
            self.activity_context = base_prompt + earlier_blk
        else:
            self.activity_context = base_prompt
    ...
    def _check_life_crisis(self) -> None:
        """Check health history for crisis conditions after weekly health evaluation.

        Called from world.py after God health evaluation and save_health_state.
        Sets self.deceased = True if physical health drops critically low for too long.
        Injects crisis-aware prompts for severe mental health deterioration.

        Crisis behavior is modulated by personality:
        - High social_seeking → prompt suggests reaching out for help
        - Low social_seeking + high emotional_stability → prompt suggests self-reliance
        - Low social_seeking + low emotional_stability → high risk, no intervention
        """
        from src.config import get_config

        health_cfg = get_config().get("world", {}).get("health", {})
        history = self.dm.read_health_history(n_weeks=10)

        if not history:
            return

        current = history[0]
        physical = current.get("physical_health", 80)
        mental = current.get("mental_health", 70)

        crisis_mental = health_cfg.get("crisis_mental_threshold", 20)
        crisis_physical = health_cfg.get("crisis_physical_threshold", 10)
        mental_consecutive = health_cfg.get("crisis_mental_consecutive_weeks", 4)
        physical_consecutive = health_cfg.get("crisis_physical_consecutive_weeks", 3)

        # --- Physical crisis: check for consecutive weeks below threshold ---
        phys_below = 0
        for h in history:
            if h.get("physical_health", 80) < crisis_physical:
                phys_below += 1
            else:
                break

        if phys_below >= physical_consecutive:
            social_seeking = self.dm.get_social_seeking()
            if social_seeking >= 60:
                # Agent seeks help → chance of survival
                self.logger.warning(
                    f"[HEALTH-CRISIS] {self.name}: physical_health < {crisis_physical} "
                    f"for {phys_below} consecutive weeks. Social support may save them."
                )
                # Don't mark deceased yet — social support gives a chance
                # Only mark if also low social_seeking or additional weeks pass
                if phys_below >= physical_consecutive + 2:
                    self.deceased = True
                    self.logger.warning(f"[HEALTH-CRISIS] {self.name} has died.")
            else:
                self.deceased = True
                self.logger.warning(
                    f"[HEALTH-CRISIS] {self.name}: physical_health < {crisis_physical} "
                    f"for {phys_below} consecutive weeks. Agent has died (low social support)."
                )

        # --- Mental crisis: track but don't force death (agent chooses) ---
        mental_below = 0
        for h in history:
            if h.get("mental_health", 70) < crisis_mental:
                mental_below += 1
            else:
                break
        ...
        if mental_below >= 4:
            self.logger.warning(
                f"[HEALTH-CRISIS] {self.name}: mental_health < {crisis_mental} "
                f"for {mental_below} consecutive weeks — severe depression risk."
            )
            # Crisis awareness is injected via health_awareness_text() in Plan/Review prompts.
            # Agent may seek help, self-destruct, or recover — personality-dependent.
            # We never force death from mental health alone.
    ...
    def judge_others(self) -> "SocialRanking":
        """Generate social ranking by scoring each person independently.

        Instead of one giant prompt with all contacts (which overflows context at
        52 weeks on 8B models), we score each person one-at-a-time with a
        lightweight prompt (persona + interactions with that person only, ~2.5K tokens).
        Python then aggregates and ranks — deterministic, no overflow risk.

        Returns:
            SocialRanking data for this agent
        """
        from src.agents.prompts import SCORE_ONE_PERSON_PROMPT
        ...
        if verify_logger:
            verify_logger.info(
                f"[VERIFY-REWARD] {self.name} judge_others (split): "
                f"scoring {len(known_names)} people individually"
            )
        ...
        affection_scores: dict[str, int] = {}
        respect_scores: dict[str, int] = {}

        for person_name in known_names:
            try:
                # Get interactions with just this one person
                interactions = self.dm.read_known_people_notes([person_name])
                if not interactions:
                    interactions = f"No recorded interactions with {person_name}."

                # Lightweight persona (not full roleplay — no scratchpad/memory/schedule)
                persona = self.dm.character_prompt()

                prompt = SCORE_ONE_PERSON_PROMPT.format(
                ...
                # Single-round call — no tools needed for a simple 0-100 score
                outputs = self._generate_with_functions(
                    messages,
                    max_rounds=1,
                    save_to_week_response=False,
                ...
                # Parse the tiny JSON: {"affection": 85, "respect": 70}
                import json
                import re

                # Extract JSON from response (may have markdown fences or extra text)
                json_match = re.search(r'\{[^{}]*"affection"[^{}]*"respect"[^{}]*\}', response, re.DOTALL)
                if json_match:
                    data = json.loads(json_match.group(0))
                    affection_scores[person_name] = max(0, min(100, int(data.get("affection", 50))))
                    respect_scores[person_name] = max(0, min(100, int(data.get("respect", 50))))
                else:
                    # Fallback: neutral scores for this person
                    if verify_logger:
                        verify_logger.warning(
                            f"[VERIFY-REWARD] {self.name} could not parse score for {person_name}, "
                            f"using neutral 50/50. Response: {response[:200]}"
                        )
                    affection_scores[person_name] = 50
                    respect_scores[person_name] = 50

            except Exception:
                # Per-person failure → neutral fallback, don't lose the whole ranking
                if verify_logger:
                    verify_logger.warning(
                        f"[VERIFY-REWARD] {self.name} failed to score {person_name}, "
                        f"using neutral 50/50"
                    )
                affection_scores[person_name] = 50
                respect_scores[person_name] = 50

        save_feature_generation(
            messages=[{"role": "user", "content": f"Split evaluation of {len(known_names)} people"}],
            output=f"affection={affection_scores}\nrespect={respect_scores}",
        ...
        if verify_logger:
            verify_logger.info(
                f"[VERIFY-REWARD] {self.name} judge_others DONE: "
                f"scored {len(affection_scores)} people"
            )
